                            ...
                            <tbody>
                                ...
                                @foreach ($tahfidzan as $row)
                                    <tr>
                                        ...
                                        <td>
                                            <input type="text" class="border-0 w-auto inputText" data-id="{{ $row->id }}" data-id-ustad="{{ $row->id_ustad }}" name="catatanDeresanA" id="catatan-deresan-a" value="{{ isset($row->catatan) ? $row->catatan : 0 }}">
                                        </td>

                                        <td>
                                            <p id="jml-{{ $row->id }}"></p>
                                            <input type="number" class="border-0 w-auto inputNumber" data-id="{{ $row->id }}" data-id-ustad="{{ $row->id_ustad }}" name="jumlahDeresanA" id="jumlah-deresan-a" value="{{ isset($row->jml_deresan) ? $row->jml_deresan : 0 }}">
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
<script>
    $(document).ready(function () {
        function calculateValues(rowId) {
            let jmlDeresanA = parseInt($('#capaian-awal-' + rowId).val() || 0) +
                            parseInt($('#capaian-akhir-' + rowId).val() || 0);
        ...
        let timeout;
        $(document).on('change', '.inputNumber, .inputText', function () {
            let id = $(this).data('id');
            let idUstad = $(this).data('id-ustad');
            let idWaktu = parseFloat($('input[name="idWaktu"]').val());
